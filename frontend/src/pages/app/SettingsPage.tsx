...
import { Bell, Moon, LogOut, ChevronRight, Shield, User, Palette, Info } from 'lucide-react';
...
function SettingRow({ icon, label, description, right }: { icon: React.ReactNode; label: string; description?: string; right?: React.ReactNode }) {
  return (
    <div className="flex items-center gap-4 py-4 border-b border-neutral-50 last:border-0">
      <div className="w-9 h-9 rounded-xl bg-neutral-100 flex items-center justify-center flex-shrink-0 text-neutral-600">
        {icon}
      </div>
  ...
  return (
    <div className="p-6 lg:p-8 max-w-2xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="card p-5">
        <h3 className="text-xs font-semibold text-neutral-400 uppercase tracking-wider mb-2">Account</h3>
        <SettingRow
          icon={<User size={16} />}
          label={user?.name ?? 'Your Account'}
          description={user?.email ?? ''}
          right={
            <button onClick={() => navigate('/profile')} className="text-neutral-400 hover:text-neutral-600 transition-colors">
      ...
      {/* Notifications */}
      <div className="card p-5">
        <h3 className="text-xs font-semibold text-neutral-400 uppercase tracking-wider mb-2">Notifications</h3>
        <SettingRow
          icon={<Bell size={16} />}
      ...
      {/* Appearance */}
      <div className="card p-5">
        <h3 className="text-xs font-semibold text-neutral-400 uppercase tracking-wider mb-2">Appearance</h3>
        <SettingRow
          icon={<Moon size={16} />}
          label="Dark Mode"
          description="Coming soon"
          right={<ToggleSwitch id="dark-mode" checked={darkMode} onChange={(v) => { setDarkMode(v); if (v) toast('Dark mode coming soon!', { icon: '🌙' }); }} />}
        />
        <SettingRow
          icon={<Palette size={16} />}
          label="Accent Color"
          description="Emerald Green"
          right={<div className="w-6 h-6 rounded-full bg-emerald-500 border-2 border-white shadow" />}
        />
      </div>

      {/* About */}
      <div className="card p-5">
        <h3 className="text-xs font-semibold text-neutral-400 uppercase tracking-wider mb-2">About</h3>
        <SettingRow
          icon={<Info size={16} />}
          label="App Version"
