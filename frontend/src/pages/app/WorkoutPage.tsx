...
function WorkoutCard({ entry, onDelete }: { entry: WorkoutEntry; onDelete: (id: string) => void }) {
  ...
  return (
    <div className="flex items-center gap-4 p-4 rounded-xl border border-neutral-100 hover:border-neutral-200 hover:bg-neutral-50/50 transition-all group">
      <div className="w-10 h-10 rounded-xl bg-purple-50 flex items-center justify-center flex-shrink-0">
        <Dumbbell size={18} className="text-purple-600" />
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-neutral-800">{entry.exercise}</p>
        <div className="flex items-center gap-3 mt-0.5">
          <span className="text-xs text-neutral-400 flex items-center gap-1">
            <Clock size={11} /> {formatDuration(entry.duration)}
          </span>
          {entry.sets && <span className="text-xs text-neutral-400">{entry.sets} sets</span>}
          {entry.reps && <span className="text-xs text-neutral-400">× {entry.reps} reps</span>}
        </div>
        {entry.notes && <p className="text-xs text-neutral-400 mt-0.5 italic">{entry.notes}</p>}
      </div>
      <button
        onClick={handleDelete}
        disabled={isDeleting}
        className="opacity-0 group-hover:opacity-100 p-1.5 rounded-lg hover:bg-red-50 text-neutral-400 hover:text-red-500 transition-all"
  ...
  return (
    <div className="p-6 lg:p-8 max-w-3xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Total Time', value: totalDuration > 0 ? formatDuration(totalDuration) : '—', color: 'text-purple-700', bg: 'bg-purple-50' },
          { label: 'Exercises', value: workouts.length.toString(), color: 'text-blue-700', bg: 'bg-blue-50' },
          { label: 'Calories Burned', value: `${Math.round(totalDuration * 7)} kcal (est)`, color: 'text-amber-700', bg: 'bg-amber-50' },
