    ...
    <div className="p-6 lg:p-8 max-w-5xl mx-auto space-y-8 animate-fade-in">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-neutral-500 dark:text-neutral-400 mt-1 text-sm lg:text-base capitalize">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card p-6 space-y-5">
          ...
          <div className="flex items-center gap-6">
            <div className="relative w-24 h-24 flex-shrink-0">
              <svg className="w-24 h-24 -rotate-90" viewBox="0 0 36 36">
                <circle cx="18" cy="18" r="15.9" fill="none" className="stroke-neutral-100 dark:stroke-neutral-800" strokeWidth="3" />
                <circle
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                <span className="text-lg font-bold text-neutral-900 dark:text-neutral-100">{calcPercent(calConsumed, calTarget)}%</span>
                <span className="text-[10px] text-neutral-400 dark:text-neutral-500">of goal</span>
              </div>
            </div>
            <div className="space-y-1">
              <div className="flex items-baseline gap-1">
                <span className="text-3xl font-bold text-neutral-900 dark:text-neutral-100">{calConsumed}</span>
                <span className="text-sm text-neutral-400 dark:text-neutral-500">/ {calTarget} kcal</span>
              </div>
              <p className="text-sm text-neutral-500 dark:text-neutral-400">
                {remaining > 0 ? (
                  <><span className="text-amber-600 dark:text-amber-400 font-semibold">{remaining} kcal</span> remaining</>
                ) : (
                  <span className="text-emerald-600 dark:text-emerald-400 font-semibold">Daily goal reached! 🎉</span>
                )}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <p className="text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">Recent Food Entries</p>
            {nutritionLog && nutritionLog.entries && nutritionLog.entries.length > 0 ? (
              nutritionLog.entries.slice(0, 3).map(entry => (
                <div key={entry.id} className="flex items-center justify-between py-1.5 border-b border-neutral-100 dark:border-neutral-800/80 last:border-0">
                  <div>
                    <p className="text-sm font-medium text-neutral-800 dark:text-neutral-200">{entry.foodName}</p>
                    <p className="text-xs text-neutral-400 dark:text-neutral-500 capitalize">{entry.mealType} · {entry.quantity}</p>
                  </div>
                  <span className="text-sm font-semibold text-neutral-700 dark:text-neutral-300">{entry.calories} kcal</span>
                </div>
              ))
            ) : (
              <p className="text-xs text-neutral-400 dark:text-neutral-500 py-2">No food logged yet today. Click 'Log Food' below to get started.</p>
            )}
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="card p-5 border-l-4 border-l-emerald-500">
            <div className="flex items-start gap-3">
              <div className="w-9 h-9 rounded-xl bg-emerald-100 dark:bg-emerald-950/60 flex items-center justify-center flex-shrink-0 mt-0.5">
                <Zap size={18} className="text-emerald-600 dark:text-emerald-400" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1.5">
                  <p className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">AI Insight</p>
                </div>
                <p className="text-sm text-neutral-700 dark:text-neutral-300 leading-relaxed">
                  {dashboard.aiInsight || 'Stay hydrated and keep tracking your meals for personalized recommendations!'}
                </p>
                <Link to="/ai-coach" className="inline-flex items-center gap-1 text-xs text-emerald-600 dark:text-emerald-400 font-semibold mt-3 hover:text-emerald-700 dark:hover:text-emerald-300 transition-colors">
          ...
          <div className="card p-5">
            ...
            <div className="space-y-3">
              {[
                { label: 'Workout', value: workoutToday > 0 ? formatDuration(workoutToday) : 'Not logged', icon: <Dumbbell size={16} />, color: 'text-purple-600 dark:text-purple-400', bg: 'bg-purple-50 dark:bg-purple-950/50' },
                { label: 'Water Intake', value: formatWater(waterConsumed), icon: <Droplets size={16} />, color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-50 dark:bg-blue-950/50' },
                { label: 'Sleep Duration', value: sleepToday ? `${sleepToday} hrs` : 'Not logged', icon: <Moon size={16} />, color: 'text-teal-600 dark:text-teal-400', bg: 'bg-teal-50 dark:bg-teal-950/50' },
                { label: 'Current Weight', value: currentWeight ? `${currentWeight} kg` : 'Not set', icon: <TrendingUp size={16} />, color: 'text-rose-600 dark:text-rose-400', bg: 'bg-rose-50 dark:bg-rose-950/50' },
